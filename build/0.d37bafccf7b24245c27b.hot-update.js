...
const filter=(root,fn)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i],{val}=get(root,key);console.log(val);if(!fn(val,key))result=unset(result,key);}}return result;};
