...
const map=(root,fn)=>{let v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){console.log({result,key:v[i]});const key=v[i],val=get(result,key);result=set(result,key,fn(val,key));// maybe some sort of setMutable?
}}return result;};
