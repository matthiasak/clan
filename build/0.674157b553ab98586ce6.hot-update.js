...
const get=(root,key)=>{let h=hash(key),r=root;console.log(r);for(let i=0;i<4;i++){r=r[frag(h,i)];console.log({frag:frag(h,i),r});}return r?r.val:undefined;};
...
const filter=(root,fn)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i],{val}=get(result,key);console.log({hash:hash(key),parts:Array(4).fill(1).map((x,i)=>{return frag(hash(key),i);}),key,val,result});if(!fn(val,key))result=unset(result,key);}}return result;};
