...
const map=(root,fn)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const{val,key}=get(root,v[i]);result=set(result,key,fn(val,key));// maybe some sort of setMutable?
}}return result;};
...
const filter=(root,fn,asArray=true)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i];if(asArray&&key==='length')continue;const{val}=get(root,v[i]);console.log({i,len,v:v[i],val,key});if(!fn(val))result=unset(result,key);}}return result;};
