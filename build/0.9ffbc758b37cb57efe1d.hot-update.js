...
const map=(root,fn)=>{let v=root.keys,result=root;if(v){console.log({v,root});for(var i=0,len=v.length;i<len;i++){const key=v[i],val=get(root,key);result=set(result,key,fn(val,key));// maybe some sort of setMutable?
}}return result;};
