...
const filter=(root,fn)=>{let v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i],val=get(result,key);if(!fn(val,key)){result=unset(result,key);}}}return result;};
