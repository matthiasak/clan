...
"use strict";
var _extends=Object.assign||function(target){for(var i=1;i<arguments.length;i++){var source=arguments[i];for(var key in source){if(Object.prototype.hasOwnProperty.call(source,key)){target[key]=source[key];}}}return target;};var _this=this;// compute the hamming weight
...
_n=n;n.keys=Array.from(root.keys||[]);for(let x=0,_o=root;x<4-delta;x++){let __n=node(),// create new child node
...
};const set=(root,key,val)=>{let[parent,child]=branch(root,hash(key));child.val=val;child.key=key;parent.keys.push(key);return parent;};
/* harmony export (immutable) */ exports["set"] = set;
const unset=(root,key)=>{let h=hash(key),[parent,child]=branch(root,h,-1);delete child[frag(h,3)];parent.keys.splice(parent.keys.indexOf(key),1);return parent;};
/* harmony export (immutable) */ exports["unset"] = unset;
const get=(root,key)=>{let h=hash(key),_r=root;for(let i=0;i<4;i++){_r=_r[frag(h,i)];if(!_r)return undefined;}return _r.val;};
/* harmony export (immutable) */ exports["get"] = get;
const map=(root,fn,asArray=true)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i];if(asArray&&key==='length')continue;const{val}=get(root,key);result=set(result,key,fn(val,key));// maybe some sort of setMutable?
}}return result;};
/* harmony export (immutable) */ exports["map"] = map;
const reduce=(root,fn,acc,asArray=true)=>{const v=root.keys;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i];if(asArray&&key==='length')continue;const{val}=get(root,key);acc=fn(acc,val,key);}}return acc;};
/* harmony export (immutable) */ exports["reduce"] = reduce;
const filter=(root,fn,asArray=true)=>{const v=root.keys,result=root;console.log(v,result);if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i];if(asArray&&key==='length')continue;const{val}=get(root,key);if(!fn(val))result=unset(result,key);}}return result;};
/* harmony export (immutable) */ exports["filter"] = filter;
const push=(root,val)=>{const length=_this.get('length')||0;return set(set(root,length,val),'length',length+1);};
/* harmony export (immutable) */ exports["push"] = push;
const pop=root=>{const length=_this.get('length')||0;return set(unset(root,length-1),'length',length-1);};
