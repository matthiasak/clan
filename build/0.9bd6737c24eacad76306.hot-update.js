...
const map=(root,fn)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i],{val}=get(root,key);result=set(result,key,fn(val,key));// maybe some sort of setMutable?
}}return result;};
...
const filter=(root,fn)=>{const v=root.keys,result=root;console.log(v,result);if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i],{val}=get(root,key);if(!fn(val))result=unset(result,key);}}return result;};
/* harmony export (immutable) */ exports["filter"] = filter;
const push=(root,val)=>{const length=root.keys&&root.keys.length||0;return set(set(root,length,val),'length',length+1);};
/* harmony export (immutable) */ exports["push"] = push;
const pop=root=>{const length=root.keys&&root.keys.length||0;return set(unset(root,length-1),'length',length-1);};
