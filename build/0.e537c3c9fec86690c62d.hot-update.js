...
const filter=(root,fn)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i],{val}=get(result,key);console.log({hash:hash(key),parts:Array(4).fill(1).map((x,i)=>{return frag(hash,i);}),key,val,result});if(!fn(val,key))result=unset(result,key);}}return result;};
