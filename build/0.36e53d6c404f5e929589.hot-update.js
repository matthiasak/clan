...
const get=(root,key)=>{let h=hash(key),r=root;for(let i=0;i<4;i++){r=r[frag(h,i)];}return r?r.val:undefined;};
/* harmony export (immutable) */ exports["get"] = get;
const map=(root,fn)=>{let v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){console.log({result,key:v[i]});const key=v[i],val=get(root,key);result=set(result,key,fn(val,key));// maybe some sort of setMutable?
}}return result;};
...
const node=(val=undefined)=>{let result=Object.create(null);if(val)result.val=val;result.keys=Object.create(null);return result;};const hamt=m=>{let root=node();if(m instanceof Array){for(let i=0,len=m.length;i<len;i++)root=set(root,i,m[i]);}else if(m instanceof Object){for(let i in m)m.hasOwnProperty(i)&&(root=set(i,m[i]));}return root;};
