...
const filter=(root,fn,asArray=true)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i];if(asArray&&key==='length')continue;const{val}=get(root,key);if(!fn(val))result=unset(result,key);}}return result;};
