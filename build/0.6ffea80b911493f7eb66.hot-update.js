...
const filter=(root,fn)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i],{val}=get(root,key);if(!fn(val))result=unset(result,key);}}return result;};
