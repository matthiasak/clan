...
const map=(root,fn)=>{let v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){console.log({key:v[i],v});const key=v[i],val=get(root,key);result=set(result,key,fn(val,key));// maybe some sort of setMutable?
}}return result;};
