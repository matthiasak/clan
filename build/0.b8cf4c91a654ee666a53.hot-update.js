...
const reduce=(root,fn,acc,asArray)=>{const v=root.keys;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i];if(asArray&&key==='length')continue;const{val}=get(root,key);acc=fn(acc,val,key);}}return acc;};
