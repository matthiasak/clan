...
const map=(root,fn,asArray=true)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const key=v[i];if(asArray&&key==='length')continue;const{val}=get(root,key);console.log({key,val});result=set(result,key,fn(val,key));// maybe some sort of setMutable?
}}return result;};
/* harmony export (immutable) */ exports["map"] = map;
const reduce=(root,fn,acc)=>{const v=root.keys;if(v){for(var i=0,len=v.length;i<len;i++){const{val,key}=get(root,v[i]);acc=fn(acc,val,key);}}return acc;};
