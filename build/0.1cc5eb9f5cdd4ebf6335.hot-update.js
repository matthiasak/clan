...
const map=(root,fn,asArray=true)=>{const v=root.keys,result=root;if(v){console.log({v});for(var i=0,len=v.length;i<len;i++){const key=v[i];if(asArray&&key==='length')continue;const{val}=get(root,key);result=set(result,key,fn(val,key));// maybe some sort of setMutable?
}}return result;};
