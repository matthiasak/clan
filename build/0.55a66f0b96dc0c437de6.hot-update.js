...
const filter=(root,fn)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const{val}=get(root,v[i]);if(!fn(val))result=unset(result,key);}}return result;};
