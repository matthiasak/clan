...
const filter=(root,fn)=>{const v=root.keys,result=root;if(v){for(var i=0,len=v.length;i<len;i++){const{val,key}=get(root,v[i]);console.log({i,v:v[i],val,key});if(!fn(val))result=unset(result,key);}}return result;};
